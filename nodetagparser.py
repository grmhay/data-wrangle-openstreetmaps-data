...
# Parses the OSM XML file for node tags only and counts frequency of the node tag type
...
def count_tags(filename):
    ...
    for _, element in ET.iterparse(filename, events=("start",)):
        ...
        # if tag is a node and has key, add the tag key to tag_keys dict
        if element.tag == 'node':
            for subtag in element.iter("tag"):
                    add_nodetag(subtag.get('k'), tag_keys)
